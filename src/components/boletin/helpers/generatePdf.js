...
export const generateBoletaPDF = async (boleta, docente, membrete, fontSizeTitle, fontSize) => {
    ...
    pdf.setFont('Times', 'bold');
    pdf.setFontSize(fontSizeTitle - 4);
    pdf.text('DOCENTE:', 40, y); //420
    pdf.setFont('Times', 'normal');
    pdf.setFontSize(fontSize);

    const docenteText = docente.nombre;          // texto del docente
    const docenteXStart = 89;                   // posición inicial X 470
    ...
    const starImg = await loadImage(imgStar);
        ...
        didDrawCell: async function (data) {
            ...
            if (data.cell.raw && String(data.cell.raw.content) === String(boleta.matific)) {
                try {
                    ...
                    const textWidth = pdfs.getTextWidth(text);
                    ...
                    // 5. Dibujar el icono
                    pdfs.addImage(starImg, 'PNG', xIcon, yIcon, iconSize, iconSize);
